...
from xfuser.core.quant.hadamard import (
    ...
    build_per_device_hadamard_matrix,
    hadamard_rotate,
)
...
def _aiter_sage_v2_hadamard_matrix(block_r):
    return build_per_device_hadamard_matrix(block_r, dtype=torch.bfloat16, allow_missing_aiter=True)
...
def _aiter_fp8_hadamard_matrix(block_r):
    ...
    modules."""
    return build_per_device_hadamard_matrix(block_r, dtype=torch.bfloat16)
...
def _build_sparge_block_mask(query, key, value, is_causal, attention_kwargs, config):
    ...
    # the (output, softmax_lse) return contract. In-place copy_ -> traceable, no
    # host sync; pruned by dynamo when off (head_balance.ENABLED is a trace-time
    # constant set before compile). block_mask is [B, H, nq, nk].
